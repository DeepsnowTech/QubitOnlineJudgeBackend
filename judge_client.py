...
from python_spj import py_spj_run

import psutil

from config import TEST_CASE_DIR, JUDGER_RUN_LOG_PATH, RUN_GROUP_GID, RUN_USER_UID, SPJ_EXE_DIR, SPJ_USER_UID, SPJ_GROUP_GID, RUN_GROUP_GID,SPJ_SRC_DIR
...
class JudgeClient(object):
    def __init__(self, run_config, exe_path, max_cpu_time, max_memory, test_case_dir,
                 submission_dir, spj_version, spj_config, io_mode, spj_compile_config=None,output=False):
        ...
        if self._spj_version and self._spj_config:
            ...
            self.spj_src_path = os.path.join(SPJ_SRC_DIR, spj_compile_config["src_name"].format(spj_version=spj_version))
    ...
    def _spj(self, in_file_path, user_out_file_path):
        ...
        return py_spj_run(self.spj_src_path,command[1],command[2])
    ...
    def _judge_one(self, test_case_file_id):
        ...
        """
        with open("/code/jc_log","w") as f:
            print(kwargs,file=f)
            print(command,file=f)
        """
        
        from code_runner import run_command_with_rlimits
