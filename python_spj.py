...
def _py_spj_run(problem_id,test_in_file_path,user_out_file_path):  
    ...
    cmd_list=["python3",PY_SPJ_HOME+str(problem_id)+".py",test_in_file_path,user_out_file_path]
    ...
    try:
        with open("/code/spj_out","w") as f:
            p_handler = subprocess.Popen(["python3",PY_SPJ_HOME+str(problem_id)+".py",test_in_file_path,user_out_file_path],stdout=f)
        p_handler.wait()
...
if __name__ == "__main__":
    _py_spj_run(1001,"/code/PythonSPJ/sample.in","/code/PythonSPJ/cmd.txt")